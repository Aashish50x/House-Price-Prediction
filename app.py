...
try:
    ...
    def predict():
        if model is None:
            return jsonify({'success': False, 'error': f"Model is offline. Reason: {model_error}"})
    ...
    @app.route('/dataset')
    def dataset():
        # Load the CSV, use absolute path for Vercel compatibility
        csv_path = os.path.join(BASE_DIR, 'house_data.csv')
        try:
            df = pd.read_csv(csv_path).head(100)
        except Exception as e:
            return f"<h1>Error Loading Dataset: {str(e)}</h1>"
    ...
    @app.route('/<path:path>')
    def catch_all(path):
        return f"<h1>Vercel Initialization Crash</h1><p>The app crashed while trying to start up.</p><pre>{err_msg}</pre>", 500
